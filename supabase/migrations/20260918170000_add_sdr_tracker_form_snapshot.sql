-- A ponte autenticada entrega as respostas já gravadas para o CRM exibir
-- sessões incompletas. Eventos públicos continuam sem PII em metadata.
...
begin
  ...
  if p_action = 'tracker_pull' then
    ...
    return coalesce((select jsonb_agg(summary order by summary->>'last_activity_at', summary->>'session_id') from (
      select jsonb_build_object('session_id', s.session_id, 'lead_id', s.lead_id, 'status', s.status, 'last_step', s.last_step, 'last_step_index', s.last_step_index, 'last_activity_at', s.last_activity_at, 'completed_at', s.completed_at,
